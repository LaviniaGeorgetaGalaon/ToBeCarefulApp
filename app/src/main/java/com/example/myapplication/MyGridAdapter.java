...
public class MyGridAdapter extends ArrayAdapter {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(Integer.parseInt((String) Day_Number.getText()) == currentDay){
            Day_Number.setTextColor(Color.parseColor("#ffffff"));
        ...
        for (int i=0; i<events.size(); i++){
            ...
            && displayYear == eventCalendar.get(Calendar.YEAR)){
               arrayList.add(events.get(i).getEVENT());
                EventNumber.setText(arrayList.size()+" Evenimente");
            }
        }
    ...
    private Date ConvertStringToDate(String eventDate){
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd", aLocale);
        Date date = null;
        try{
